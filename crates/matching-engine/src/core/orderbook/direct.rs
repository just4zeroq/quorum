...
impl DirectOrderBook {
    ...
    fn try_match(&mut self, cmd: &mut OrderCommand) -> Size {
        ...
        // 获取最优价格档位
        let bucket_idx = if is_bid {
            self.ask_price_buckets.values().next().copied()
        } else {
            self.bid_price_buckets.values().next().copied()
        };
        ...
        // 检查价格是否可撮合
        let first_price = self.buckets[bucket_idx].price;
        if is_bid && first_price > limit_price {
            return 0;
        }
        if !is_bid && first_price < limit_price {
            return 0;
        ...
        // 从 head（最早订单）开始撮合，实现 FIFO
        let mut maker_idx = Some(self.buckets[bucket_idx].head);
        ...
        while let Some(idx) = maker_idx {
            ...
            let (maker_price, maker_filled, maker_size, maker_parent, maker_next) = {
                let order = &self.orders[idx];
                ...
                (order.price, order.filled, order.size, order.parent, order.next)
            };
            ...
            // 移除完成的 maker 订单（从链表中断开）
            let bucket_head = self.buckets[maker_parent].head;
            let bucket_tail = self.buckets[maker_parent].tail;
            let should_remove_bucket = bucket_head == bucket_tail; // 桶里只有这一个订单

            // 保存 next（在移除前获取）
            let next_maker = maker_next;

            // 更新 bucket head
            if bucket_head == idx {
                self.buckets[maker_parent].head = maker_next.unwrap_or_default();
            }
            if bucket_tail == idx {
                self.buckets[maker_parent].tail = maker_next.unwrap_or_default();
            }
            ...
            if should_remove_bucket {
                ...
                // 桶被移除，需要获取下一个最优桶
                current_bucket_idx = if is_bid {
                    match self.ask_price_buckets.values().next().copied() {
                        Some(idx) => idx,
                        None => {
                            self.best_ask_order = None;
                            return filled;
                        }
                    }
                } else {
                    match self.bid_price_buckets.values().next().copied() {
                        Some(idx) => idx,
                        None => {
                            self.best_bid_order = None;
                            return filled;
                        }
                    }
                };
            ...
            // 更新最优订单为当前桶的 head
            if is_bid {
                self.best_ask_order = Some(self.buckets[current_bucket_idx].head);
            } else {
                self.best_bid_order = Some(self.buckets[current_bucket_idx].head);
            }
        ...
        if filled > 0 {
            if is_bid {
                self.best_ask_order = maker_idx.or(Some(self.buckets[current_bucket_idx].head));
            } else {
                self.best_bid_order = maker_idx.or(Some(self.buckets[current_bucket_idx].head));
            }
        }
    ...
    fn insert_order(&mut self, order_idx: OrderIdx) {
        ...
        if let Some(&bucket_idx) = buckets_map.get(&price) {
            ...
            let bucket_idx = self.buckets.insert(Bucket {
                ...
                num_orders: 1,
                head: order_idx,  // 新桶的 head 和 tail 都指向第一个订单
                tail: order_idx,
            });
            ...
            if let Some(lower_idx) = lower_bucket_idx {
                ...
                self.orders[order_idx].next = old_best;
                self.orders[order_idx].prev = None;
            }
        }
    }
    ...
    fn remove_order(&mut self, order_idx: OrderIdx) -> bool {
        ...
        if self.buckets[bucket_idx].tail == order_idx {
            self.buckets[bucket_idx].tail = prev.unwrap_or_default();
        }
        // 如果是头部订单
        if self.buckets[bucket_idx].head == order_idx {
            self.buckets[bucket_idx].head = next.unwrap_or_default();
        }

        // 如果桶变空
        if self.buckets[bucket_idx].num_orders == 0 {
        ...
        if Some(order_idx) == self.best_ask_order {
            self.best_ask_order = next.or(prev);
        } else if Some(order_idx) == self.best_bid_order {
            self.best_bid_order = next.or(prev);
        }

...
import type { Balance, Asset, WalletHistory } from '../types';
...
export default function WalletPage() {
  ...
  const [history, setHistory] = useState<WalletHistory>({ deposits: [], withdrawals: [] });
      ...
      <div className="flex space-x-4 border-b border-gray-700">
        {(['deposit', 'withdraw', 'history'] as const).map((tab) => (
          <button
            ...
            className={`px-6 py-3 text-white border-b-2 capitalize ${
              activeTab === tab
      ...
      {activeTab === 'deposit' && (
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          <p className="text-yellow-400 mt-4 text-sm">
            Send only {selectedAsset} ({selectedNetwork}) to this address. Do not send other assets.
          </p>
        </div>
      )}
 ...
 */
function HistoryTab({ history }: { history: WalletHistory }) {

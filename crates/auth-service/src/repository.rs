...
use crate::models::{UserSession, ApiKey};
...
impl AuthRepository {
    ...
    pub async fn upsert_session(&self, session: &UserSession) -> Result<()> {
        sqlx::query(
            r#"
    ...
    pub async fn get_session_by_token(&self, token_hash: &str) -> Result<Option<UserSession>> {
        let session = sqlx::query_as::<_, UserSession>(
    ...
    pub async fn update_api_key_used(&self, key_id: &str) -> Result<()> {

...
impl super::OrderBook for NaiveOrderBook {
    ...
    fn get_l2_data(&self, depth: usize) -> L2MarketData {
        ...
        // 买单（从高到低）- 需要反向迭代
        let mut bid_prices: Vec<_> = self.bid_buckets.keys().collect();
        bid_prices.sort_by(|a, b| b.cmp(a)); // 降序
        for price in bid_prices.iter().take(depth) {
            if let Some(bucket) = self.bid_buckets.get(price) {
